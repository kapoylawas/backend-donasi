...
<main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-300">
    <div class="container px-6 py-8 mx-auto">
        ...
        <div class="p-6 bg-white rounded-md shadow-md">
            <h2 class="text-lg font-semibold text-gray-700 capitalize">UPLOAD SLIDER</h2>
            <hr class="mt-4">
            <form action="{{ route('admin.slider.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="grid grid-cols-1 gap-6 mt-4">
                    ...
                    <div>
                        ...
                        <input class="w-full mt-2 bg-gray-200 rounded-md form-input focus:bg-white" type="text" name="link" value="{{ old('link') }}" placeholder="Link Promo">
                        @error('link')
                            <div class="w-full mt-2 overflow-hidden bg-red-200 rounded-md shadow-sm">
                                <div class="px-4 py-2">
                                    <p class="text-sm text-gray-600">{{ $message }}</p>
                ...
                <div class="flex justify-start mt-4">
                    <button type="submit" class="px-4 py-2 text-gray-200 bg-gray-600 rounded-md hover:bg-gray-700 focus:outline-none focus:bg-gray-700">UPLOAD</button>
                </div>
            </form>
        </div>
                        ...
                        @forelse($sliders as $slider)
                            <tr class="bg-white border">
                                ...
                                <td class="flex justify-center px-16 py-2">
                                    <img src="{{ $slider->image }}" class="rounded object-fit-cover" style="width: 35%">
                                </td>
                                <td class="px-16 py-2">
                                    {{ $slider->link }}
                                </td>
                                <td class="px-10 py-2 text-center">
                                    <button onClick="destroy(this.id)" id="{{ $slider->id }}" class="px-4 py-2 text-xs text-white bg-red-600 rounded shadow-sm focus:outline-none">HAPUS</button>
                                </td>
                            </tr>
                        @empty
